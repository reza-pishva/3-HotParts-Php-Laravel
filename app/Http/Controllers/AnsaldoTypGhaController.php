...
class AnsaldoTypGhaController extends Controller
{
    ...
    */
    public function delete($id){
        ...
        $n3= DB::table('ansaldo_out_ghataats')->where('ID_TG',$id)->get()->count();
        $n4= DB::table('ansaldo_send_bazsazi_ghataats')->where('ID_TG',$id)->get()->count();
        $n5= DB::table('ansaldo_store_program_ins')->where('ID_TG',$id)->get()->count();
        $n=$n1+$n2+$n3+$n4+$n5;
        if($n==0){
            Ansaldo_type_ghataat::where('ID_TG', $id)->delete();
