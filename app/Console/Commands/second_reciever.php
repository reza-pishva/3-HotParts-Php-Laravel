...
class second_reciever extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'second:reciever';
    ...
    public function handle()
    {
        ...
        $id_role=Role::where('role',"second_level_confirmation")->first()->id_role;
        $id_grs=Grouprole::where('id_role',$id_role)->get()->toArray();
        $id_users=Groupuser::where('id_gr',$id_grs[0]['id_gr'])->get()->toArray();
        $requests=Exit_goods_permission::where('level',2)->get()->toArray();
            ...
            foreach ($id_users as $id_user){
                ...
                if(true){
                    ...
                    else
                    {
                        ...
                        $values = array('date_shamsi_sms' => $date_shamsi,'id_user' => $id_user['id_user'],'description' => 'به نرم افزار مدیریت دریافت مجوز ورود و خروج بخش دریافت درخواست جدید مسئول حراست مراجعه شود','mobile' => $target[0]->mobile);
