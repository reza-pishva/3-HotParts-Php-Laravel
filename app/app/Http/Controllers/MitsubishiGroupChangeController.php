...
class MitsubishiGroupChangeController extends Controller
{
    /**
                        ...
                        foreach ($groupusers as $groupuser) {
                            ...
                            foreach ($grouproles as $grouprole) {
                                ...
                                if($role_name['role'] ==="admin" or $role_name['role'] ==="track_group_ghataat_change"){
                                    ...
                                    return view('Mitsubishi.mitsubishi_group_change',compact('requests','ghataats','sazs'));
                                }
    ...
    public function total2($id)
    {
        $ID_TGS = DB::table('mitsubishi_type_ghataats')->where('ID_TG','>',0)->get()->toArray();
        $data3 = DB::table('users')->where('id','>',0)->get()->toArray();
        $data = DB::table('mitsubishi_group_names')->where('ID_TG',$id)->orderBy('ID_G', 'DESC')->get()->toArray();
        return response()->json(['results'=> $data,'ID_TGS'=>$ID_TGS,'ID_USERS'=>$data3]);//,'ID_USERS'=>$ID_USERS
    }
