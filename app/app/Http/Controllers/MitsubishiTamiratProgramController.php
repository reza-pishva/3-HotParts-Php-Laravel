...
class MitsubishiTamiratProgramController extends Controller
{
    /**
    ...
    public  function store(Request $request){
        ...
        $atp= new Mitsubishi_tamirat_program();
        ...
        if($request->file('select_file')){
            ...
            {
                if (DB::table('mitsubishi_tamirat_programs')->where('ID_T','>',0)->exists()){
                    $id_t = (DB::table('mitsubishi_tamirat_programs')->where('ID_T','>',0)->orderBy('ID_T', 'DESC')->first()->ID_T)+1;
                }
                else{
                    $id_t = 1;
                }
...
//           'class_name'  => 'alert-success'
                                ...
                                foreach ($groupusers as $groupuser) {
                                    ...
                                    foreach ($grouproles as $grouprole) {
                                        ...
                                        if($role_name['role'] ==="admin" or $role_name['role'] ==="track_create_program"){
                                            ...
                                            $requests=Mitsubishi_tamirat_program::all();
                                            ...
                                            return view('Mitsubishi.mitsubishi_tamirat_program',compact('requests','anns','auns','atts','ats'));
                                        }
    ...
    public function total()
    {
        ...
        $data = DB::table('mitsubishi_tamirat_programs')->where('ID_T','>',0)->orderBy('ID_T', 'DESC')->get()->toArray();
        return response()->json(['results'=> $data,'ID_UNS'=>$ID_UNS,'ID_TAS'=>$ID_TAS,'ID_TTS'=>$ID_TTS,'ID_USERS'=>$data3]);//,'ID_USERS'=>$ID_USERS
    }
    public function delete($id){
        $n= Mitsubishi_savabegh::where('ID_T', $id)->where('SAV_TYPE','T')->get()->count();
        if($n==0){
            $id_t = DB::table('mitsubishi_tamirat_programs')->orderBy('ID_T', 'DESC')->first()->ID_T;
            Mitsubishi_tamirat_program::where('ID_T', $id)->delete();
    ...
    public function edit(Request $request)
    {
        ...
        $FILE_NAME_EDIT=DB::table('mitsubishi_tamirat_programs')->where('ID_T',$ID_T)->orderBy('ID_T', 'DESC')->first()->FILE_NAME;
        $validation = Validator::make($request->all(), [
        ...
        if($request->file('select_file_edit')){
            ...
            Mitsubishi_tamirat_program::where('ID_T', $ID_T)->update([
    ...
    public function report_queryp(Request $request)
    {
        ...
        $query="SELECT * FROM mitsubishi_tamirat_programs WHERE ".$query1." AND ".$query2." AND ".$query3." AND ".$query6." AND ".$query7." ORDER BY ID_T DESC";
    ...
    public function report_queryp3($id)
    {
        ...
        $ID_TAS = DB::table('mitsubishi_tamirkarans')->where('ID_TA','>',0)->get()->toArray();
        $data3 = DB::table('users')->where('id','>',0)->get()->toArray();
        $data = DB::table('mitsubishi_tamirat_programs')->where('ID_T',$id)->get()->toArray();
        return response()->json(['results'=> $data,'ID_UNS'=>$ID_UNS,'ID_TAS'=>$ID_TAS,'ID_TTS'=>$ID_TTS,'ID_USERS'=>$data3]);//,'ID_USERS'=>$ID_USERS
    }
...
//        $jDate=\Morilog\Jalali\CalendarUtils::toJalali($year, $month, $day);
        ...
        $requests=Mitsubishi_buy_ghataat::all();
        foreach($requests as $request):
            ...
            Mitsubishi_buy_ghataat::where('ID_T',$request->ID_T)->update(['DATE_SHAMSI'=>$date_shamsi]);
...
//        $date_shamsi=implode("",$jDate);
    ...
    {
        $data = DB::table('m_savabegh_total_view')->where('ID_T',$id)->where('SAV_TYPE','T')->get()->toArray();
