...
class AnsaldoStoreProgramInController extends Controller
{
    /**
    ...
    public  function store(Request $request){
        ...
        $atp= new Ansaldo_store_program_in();
                                                ...
                                                foreach ($groupusers as $groupuser) {
                                                    ...
                                                    foreach ($grouproles as $grouprole) {
                                                        ...
                                                        if($role_name['role'] ==="admin" or $role_name['role'] ==="track_create_program"){
                                                            ...
                                                            return view('Ansaldo.ansaldo_store_program_ins',compact('requests','ghataats'));
                                                        }
...
//        $ID_BAS = DB::table('ansaldo_bazsazs')->where('ID_BA','>',0)->get()->toArray();
        ...
        $data = DB::table('ansaldo_store_program_ins')->where('ID_T','>',0)->orderBy('ID_T', 'DESC')->get()->toArray();
...
//        $ID_BAS = DB::table('ansaldo_bazsazs')->where('ID_BA','>',0)->get()->toArray();
        $data3 = DB::table('users')->where('id','>',0)->get()->toArray();
        $data = DB::table('ansaldo_store_program_ins')->where('ID_T','>',0)->where('ID_USER',$id_user)->where('DATE_SHAMSI','>=',$current_date_shamsi)->orderBy('ID_T', 'DESC')->get()->toArray();
    ...
    public function onlyone()
    {
        $id_user = auth()->user()->id;
        $ID_TGS = DB::table('ansaldo_type_ghataats')->where('ID_TG','>',0)->get()->toArray();
        $ID_T=Ansaldo_store_program_in::where('id_user',$id_user)->orderBy('ID_T', 'desc')->first()->ID_T;
        $data3 = DB::table('users')->where('id','>',0)->get()->toArray();
        $data = DB::table('ansaldo_store_program_ins')->where('ID_T',$ID_T)->get()->toArray();
    ...
    public function onlyone2($id)
    {
        ...
        $ID_TGS = DB::table('ansaldo_type_ghataats')->where('ID_TG','>',0)->get()->toArray();
        $data3 = DB::table('users')->where('id','>',0)->get()->toArray();
        $data = DB::table('ansaldo_store_program_ins')->where('ID_T',$id)->get()->toArray();
        return response()->json(['results'=> $data,'ID_TGS'=>$ID_TGS,'ID_USERS'=>$data3]);//,'ID_USERS'=>$ID_USERS
    }
    public function delete($id){
        $n= DB::table('ansaldo_savabeghs')->where('SAV_TYPE','A')->where('ID_T',$id)->get()->count();
        $rec_no = DB::table('ansaldo_store_program_outs')->where('ID_T',$id)->get()->count();
        if($n==0){
            if($rec_no>0){
                return response()->json(['success'=>'hi','perm'=>0,'rec_no'=>$rec_no,'id_t'=>$id,'n'=>$n]);
            }else{
                Ansaldo_store_program_in::where('ID_T', $id)->delete();
                return response()->json(['success'=>'hi','perm'=>0,'rec_no'=>$rec_no,'id_t'=>$id,'n'=>$n]);
            }
        }else{
            return response()->json(['success'=>'hi','perm'=>0,'rec_no'=>$rec_no,'id_t'=>$id,'n'=>$n]);
    ...
    public function edit(Request $request)
    {
        ...
        Ansaldo_store_program_in::where('ID_T', $ID_T_EDIT)->update([
            'ID_TG'=>$ID_TG_EDIT,
    ...
    public function report_queryp(Request $request)
    {
        ...
        $query="SELECT * FROM ansaldo_store_program_ins WHERE ".$query2." AND ".$query3." AND ".$query6." ORDER BY ID_T DESC";
        //$query="SELECT * FROM ansaldo_store_program_ins WHERE ORDER BY ID_T DESC";
    ...
    public function report_queryp3(Request $request)
    {
        ...
        Querytext::where('id_user', $id_user)->delete();
        if($RESV_R==0){
            $query="SELECT * FROM ansaldo_anbar_send_recieve_view WHERE ".$query2." AND ".$query3."  ORDER BY ID_T DESC";
        }
        if($RESV_R==1){
            $query="SELECT * FROM ansaldo_anbar_send_recieve_view WHERE ".$query2." AND ".$query3."  AND COUNT_GH = GROUP_COUNT ORDER BY ID_T DESC";
        }
        if($RESV_R==2){
            $query="SELECT * FROM ansaldo_anbar_send_recieve_view WHERE ".$query2." AND ".$query3." AND COUNT_GH != GROUP_COUNT ORDER BY ID_T DESC";
        }
    ...
    {
        $data = DB::table('ansaldo_store_program_outs')->where('ID_SUB',$id)->get()->toArray();
